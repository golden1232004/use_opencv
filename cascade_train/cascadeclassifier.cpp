                                ...
                                bool baseFormatSave )
{
    ...
    for( int i = startNumStages; i < numStages; i++ )
    {
        ...
        // update and add the positive/negative samples into the training set.
...
/** 
 * predict(sampleIdx) use the cascaded classfiers to predict the 'sampleIdx'th sample. If there is
 * none classifier, all samples are taken as positive.
 * @param sampleIdx The sample's index.
 * 
...
/** 
 * updateTrainingSet use the current cascade classifiers to predict the pos/neg samples, kick out
 * those predicted as negative and add more negative samples if necessary.
 * @param acceptanceRatio used to store the current false negative ratio.
 * 
...
bool CvCascadeClassifier::updateTrainingSet( double& acceptanceRatio)
{
    ...
    // 重新计算需要的负样本个数，使得proNumNeg/numNeg ~ posCount/numPos，也就是保持正、负样本数量大致相当。
...
/** 
 * fillPassedSamples take those samples predicted as postivie as the training data for the next
 * layer.
 * @param first      The first index in the featureEvaluator to store the passed data.
 * @param count      The number of samples to be added.
 * @param isPositive 
 * @param consumed   The number of scaned samples.
 * 
 * @return the hit number of samples.
 */
