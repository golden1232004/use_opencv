...
bool CvHaarFeatureParams::scanAttr( const string prmName, const string val)
{
    if ( !CvFeatureParams::scanAttr( prmName, val ) )
    {
        if( !prmName.compare("-mode") )
        {
            ...
            if (mode == -1)
                return false;
            // TODO: I think there should be a statement of "return true" .
        }
        return false;
    }
    return true;
...
/** 
 * setImage update the corresponding 'sum', 'tilted' and 'normfactor' in according to
 * the 'img'.
 * @param img      
 * @param clsLabel 
 * @param idx      index in the training samples set.
 */
...
/**
 * @param offset The step of the integral image.
 */
