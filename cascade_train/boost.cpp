                           ...
                           const CvCascadeBoostParams& _params )
{
    ...
    do
    {
        ...
        if( !tree->train( data, subsample_mask, this ) ) //subsample_mask: 1xN. Indicats if a sample should participate in the next training.
        {
            delete tree;
            break;
        }
        cvSeqPush( weak, &tree );
        update_weights( tree );
        // Trim some samples out of the next training by according to the weights and weight_trim_rate. 
        trim_weights();
...
/** 
 * update_weights init or update pos/neg samples' weights and set the weight of the tree, 
 * i.e., the weak classifier.
 * @param tree if zero, init the weights. otherwise, update the weights.
 */
void CvCascadeBoost::update_weights( CvBoostTree* tree )
{
    ...
    if( !tree ) // before training the first tree, initialize weights and other parameters
    {
        ...
        // TODO: what is the meaning of is_buf_16u?
        if (data->is_buf_16u)
        {
    ...
    else
    {
        ...
        if( have_subsample ) // some samples may be trimed(subtracted) by the previous trim_weights(),
                             // i.e., they are not participated in the training of the most recent weak
                             // classifier.
        {
...
/** 
 * isErrDesired calculate the 'threshold', the hit rate and the false alarm rate.
 * 
 * @return true if the false alarm rate is less than the maximum false alarm rate.
 */
bool CvCascadeBoost::isErrDesired()
{
    ...
    for( int i = 0; i < sCount; i++ )
        if( ((CvCascadeBoostTrainData*)data)->featureEvaluator->getCls( i ) == 1.0F ) // is positive
            eval[numPos++] = predict( i, true );
    
    // sort 'eval' ascending order.
    std::sort(&eval[0], &eval[0] + numPos);
    // let the minHitRate*numPos number of positives to be passed.
    ...
    for( int i = thresholdIdx - 1; i >= 0; i--)
        if ( abs( eval[i] - threshold) < FLT_EPSILON ) // take into those which are almost same as the threshold.
            numPosTrue++;
    float hitRate = ((float) numPosTrue) / ((float) numPos);

    for( int i = 0; i < sCount; i++ )
    {
        if( ((CvCascadeBoostTrainData*)data)->featureEvaluator->getCls( i ) == 0.0F ) // is negative
        {
            numNeg++;
            if( predict( i ) )
                numFalse++;
